...
export const addFood = asyncHandler(async (req, res) => {
    ...
    const donorId = req.user.id; // Extracted from JWT
...
export const getAvailableFood = asyncHandler(async (req, res) => {
    const availableFood = await Food.find({ status: "Available" }).populate("donor", "name phone");
...
export const deleteFood = asyncHandler(async (req, res) => {
    ...
    if (food.donor.toString() !== donorId) {
        return res.status(403).json({ message: "You can only delete your own food posts" });
    }
