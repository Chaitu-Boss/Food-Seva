...
export const signup = asyncHandler(async (req, res) => {
  ...
  if (user) {
    ...
    res.status(201).json({ _id: user._id, name: user.name, email: user.email, phone: user.phone, token });
  } else {
    res.status(400);
...
export const login = asyncHandler(async (req, res) => {
  ...
  if (user && (await user.matchPassword(password))) {
    ...
    res.status(200).json({ _id: user._id, name: user.name, email: user.email, phone: user.phone, token });
  } else {
    res.status(401);
...
export const updatePassword = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    const updatedUser = await User.findOneAndUpdate(
      { $or: [{ email: identifier }, { phone: identifier }] }, // Check for either email or phone
